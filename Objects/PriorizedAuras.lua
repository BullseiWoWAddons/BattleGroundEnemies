...
BattleGroundEnemies.Objects.PriorizedAurasContainer = {}
...
function BattleGroundEnemies.Objects.PriorizedAurasContainer.New(playerButton, type)

	
	local PriorizedAurasContainer = CreateFrame("Frame", nil, playerButton)
	PriorizedAurasContainer.Active = {}
	PriorizedAurasContainer.Inactive = {}
	PriorizedAurasContainer.type = type

	PriorizedAurasContainer:SetScript("OnHide", function(self) 
		self:SetWidth(0.001)
		self:SetHeight(0.001)
	end)
	
	PriorizedAurasContainer:Hide()
	
	PriorizedAurasContainer.SetPosition = function(self, point, relativeTo, relativePoint, offsetX, offsetY)
		self:ClearAllPoints()
	...
	PriorizedAurasContainer.Reset = function(self)
		for identifier, auraFrame in pairs(self.Active) do
			auraFrame.Cooldown:Clear()
		end
	end
	
	PriorizedAurasContainer.ApplySettings = function(self)
	...
	PriorizedAurasContainer.AuraPositioning = function(self)
		local conf = playerButton.bgSizeConfig
	...
	PriorizedAurasContainer.SetContainerPosition = function(self)
		local conf = playerButton.bgSizeConfig
		if self.type == "buff" then
	...
	PriorizedAurasContainer.DisplayAura = function (self, spellID, srcName, amount, duration, expirationTime, debuffType)
	...
	PriorizedAurasContainer.Positioning = function(self, iconSize, verticalGrowdirection, horizontalGrowdirection, framesPerRow, horizontalSpacing, verticalSpacing)
	...
	-- PriorizedAurasContainer:Show()
	-- PriorizedAurasContainer:SetBackdrop({
		-- bgFile = "Interface/Buttons/WHITE8X8", --drawlayer "BACKGROUND"
		-- edgeFile = 'Interface/Buttons/WHITE8X8', --drawlayer "BORDER"
		-- edgeSize = 1
	-- })
	-- PriorizedAurasContainer:SetBackdropColor(1, 1, 1, 1)
	--PriorizedAurasContainer:SetSize(50,50)
	
	return PriorizedAurasContainer	
end
